...
def Conv_2D_Block(inputs, num_filters, kernel, strides):
    # 2D Convolutional Block with BatchNormalization
    x = tf.keras.layers.Conv2D(num_filters, (kernel, kernel), strides=(strides, strides), padding="same", kernel_initializer="he_normal")(inputs)
...
class SEResNet:
    def __init__(self, length, width, num_channel, num_filters, ratio=16, problem_type='Regression', output_nums=1,
...
if __name__ == '__main__':
    ...
    Model = SEResNet(length, width, num_channel, model_width, ratio=reduction_ratio, problem_type=problem_type,
