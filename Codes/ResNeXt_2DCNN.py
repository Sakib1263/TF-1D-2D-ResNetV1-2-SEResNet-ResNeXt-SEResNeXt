...
def Conv_2D_Block(x, model_width, kernel, strides):
    ...
    x = tf.keras.layers.Conv2D(model_width, (kernel, kernel), strides=(strides, strides), padding="same", kernel_initializer="he_normal")(x)
...
def grouped_convolution_block(inputs, num_filters, kernel_size, strides, cardinality):
    ...
    for c in range(cardinality):
        ...
        x = Conv_2D_Block(x, grouped_channels, kernel_size, strides=strides)
...
def residual_block_bottleneck(inputs, num_filters, cardinality):
    ...
    # x        : input into the block
    # n_filters: number of filters
    shortcut = Conv_2D_Block(inputs, num_filters * 2, 1, 1)
    #
    x = grouped_convolution_block(inputs, num_filters, 3, 1, cardinality)
    x = Conv_2D_Block(x, num_filters * 2, 1, 1)
    #
...
class ResNeXt:
    def __init__(self, length, num_channel, num_filters, cardinality=8, problem_type='Regression',
